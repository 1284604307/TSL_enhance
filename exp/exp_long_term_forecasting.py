    ...
    def trainOne(self, train_loader):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
            ...
            if self.args.use_amp:
                ...
                loss.backward()
                model_optim.step()
            preds = preds + outputs.detach().cpu().tolist()
            reals = reals + batch_y.detach().cpu().tolist()
        # if (i + 1) % 100 == 0:
        ...
        # drawUtil.completeMSE(np.reshape(reals, (-1)), np.reshape(preds, (-1)))
        # drawUtil.drawResultCompare(np.reshape(reals, (-1)), np.reshape(preds, (-1)),
